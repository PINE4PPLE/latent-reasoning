...
from transformers import AutoModelForCausalLM, BitsAndBytesConfig, Qwen2ForCausalLMWithLatent
...
class LatentActor(Actor):
    def __init__(self,
        ...
        **kwargs,
        ...
        if isinstance(pretrain_or_model, str):
            ...
            nf4_config = None
            model_class = Qwen2ForCausalLMWithLatent
    ...
    def generate(self, input_ids: torch.Tensor, **kwargs) -> Union[
        ...
        generate_args = {
            ...
            "eos_token_id": kwargs.get("eos_token_id"),
            "pad_token_id": kwargs.get("pad_token_id"),
            "change_token_ids": kwargs.get("change_token_ids"),
            "max_length": kwargs.get("max_length"),
            "max_latent_length": kwargs.get("max_latent_length"),
        }
        ...
        # Call generate
        sequences, generated_weights, generated_target_number = self.model.generate(**generate_args)

        # Prepare mask tensor
        eos_token_id = generate_args["eos_token_id"]
        pad_token_id = generate_args["pad_token_id"]

        sequences, attention_mask, action_mask = self.process_sequences(sequences, generated_weights, input_ids.size(1), eos_token_id, pad_token_id)
        return sequences, attention_mask, action_mask, generated_weights, generated_target_number
    
    def process_sequences(self, sequences: torch.Tensor, generated_weights, input_len, eos_token_id, pad_token_id):
        attention_mask = (sequences[:,:,0].ne(eos_token_id) & sequences[:,:,0].ne(pad_token_id) & generated_weights[:,:,0].eq(1.0)).to(dtype=torch.long)
        seq_length = attention_mask.size(1)
        ...
        # in RL, state_i (current token) + action_i (next token) -> state_i+1 (next token)
        state_seq = sequences[:, input_len - 1 : -1, :]
        action_mask = state_seq[:,:,0].ne(eos_token_id) & state_seq[:,:,0].ne(pad_token_id)
        action_mask[:, 0] = 1
    ...
    def forward(
        self,
        sequences: torch.LongTensor,
        generated_weights: torch.LongTensor, 
        generated_target_num: torch.LongTensor,
        num_actions: Optional[Union[int, list[int]]] = None,
        attention_mask: Optional[torch.Tensor] = None,
        return_output=False,
        logps_allgather=False,
        ...
        output = self.model.probability_estimation(sequences, generated_weights, generated_target_num, attention_mask)
        # https://github.com/OpenRLHF/OpenRLHF/pull/634
        # output["logits"] = output["logits"].to(torch.float32)
        ...
        if num_actions is None:
            assert return_output
            return output
        action_log_probs = torch.log(output)
